...
async def ai_settings(message: types.Message):
    ...
    # Создаем список уникальных моделей из всех провайдеров
    all_models = set()
    for provider_info in PROVIDER_HIERARCHY:
        for model in provider_info['models']:
            all_models.add(model)
    
    # Сортируем модели для лучшей презентации
    sorted_models = sorted(list(all_models))
    
    # Создаем клавиатуру для выбора модели
    keyboard = types.InlineKeyboardMarkup(row_width=1)
    for model in sorted_models:
        # Получаем список провайдеров для этой модели
        providers = get_providers_for_model(model)
        provider_names = [p.__name__ for p in providers]
        provider_text = ", ".join(provider_names[:3])
        if len(provider_names) > 3:
            provider_text += f" и ещё {len(provider_names) - 3}"
            
        keyboard.add(
            types.InlineKeyboardButton(
                f"{'✅ ' if model == current_model else ''}{model} ({len(provider_names)} провайдеров)",
    ...
    await message.answer(
        ...
        f"🔹 Выбранная модель: {current_model}\n"
        f"🔹 Доступных провайдеров: {len(get_providers_for_model(current_model))}\n\n"
...
async def process_model_selection(callback_query: types.CallbackQuery):
    model = callback_query.data.replace('select_model_', '')
    
    # Обновляем настройки пользователя
    user_data.get_user_data(callback_query.from_user.id)['ai_settings']['model'] = model
    user_data.save()
    
    # Получаем количество провайдеров для этой модели
    providers = get_providers_for_model(model)
    
    await callback_query.message.edit_text(
        f"✅ Модель {model} успешно выбрана!\n\n"
        f"Доступно {len(providers)} провайдеров для этой модели."
    )

def get_providers_for_model(model_name):
    """Получает список провайдеров, поддерживающих указанную модель"""
    providers = []
    for provider_info in PROVIDER_HIERARCHY:
...
async def try_gpt_request(prompt: str, posts_text: str, user_id: int):
    """Пытаемся получить ответ от выбранной модели, используя подходящие провайдеры"""
    last_error = None
    ...
    # Получаем список провайдеров, поддерживающих эту модель
    suitable_providers = get_providers_for_model(current_model)
    
    # Если нет подходящих провайдеров, используем все провайдеры (на случай, если модель поддерживается неофициально)
    if not suitable_providers:
        logger.warning(f"Модель {current_model} не указана в списке поддерживаемых ни у одного провайдера")
        for provider_info in PROVIDER_HIERARCHY:
            suitable_providers.append(provider_info['provider'])
    
    # Приоритет отдаем DDG, если он поддерживает модель
    providers_to_try = []
    for provider in suitable_providers:
        if provider == g4f.Provider.DDG:
            providers_to_try.insert(0, provider)
        else:
            providers_to_try.append(provider)
    
    # Перемешиваем остальных провайдеров для равномерного распределения запросов
    if len(providers_to_try) > 1:
        # Сохраняем первый провайдер (возможно DDG)
        first_provider = providers_to_try[0]
        other_providers = providers_to_try[1:]
        random.shuffle(other_providers)
        providers_to_try = [first_provider] + other_providers
    
    # Генерируем случайный ID сессии
    session_id = ''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=32))
    
    for provider in providers_to_try:
        if provider in rate_limited_providers:
            continue
            
        try:
            logger.info(f"Пробую провайдера {provider.__name__} с моделью {current_model}")
            ...
            response = await g4f.ChatCompletion.create_async(
                model=current_model,  # Используем только выбранную модель
            ...
            logger.error(f"Ошибка с провайдером {provider.__name__}: {error_str}")
            
            if "429" in error_str or "ERR_INPUT_LIMIT" in error_str:
                rate_limited_providers.add(provider)
                logger.warning(f"Провайдер {provider.__name__} временно заблокирован")
    ...
    if len(rate_limited_providers) > 0:
        raise Exception(f"Все доступные провайдеры для модели {current_model} временно заблокированы. Попробуйте позже.")
    else:
        raise Exception(f"Не удалось получить ответ от модели {current_model} через все доступные провайдеры. Причина последней ошибки: {last_error}")
